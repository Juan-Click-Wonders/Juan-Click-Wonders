<template>
    <div class="w-full mx-auto">
        <div class="flex">
            ...
            <aside class="w-64 mt-10 p-4 border-black border-2 rounded-xl ml-4 h-fit">
                ...
                <div v-for="category in displayCategories" :key="category.category_id">
                    <label class="flex items-center space-x-2">
                        <input type="checkbox" v-model="selectedCategories" :value="category.category_id" @change="filterProducts">
            ...
            <!-- Product Grid -->
            <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 p-6">
                <router-link v-for="product in filteredProducts" :key="product.product_id" :to="'/product/' + product.product_id"
                    class="block border p-4 rounded-lg shadow-md bg-white hover:shadow-lg transition-transform transform hover:scale-105">
                    <!-- Placeholder for Product Image -->
                    <div class="h-40 bg-gray-300 flex items-center justify-center">
                        <img v-if="product.image_url" :src="getImageUrl(product.image_url)" :alt="product.name" class="max-h-full max-w-full object-contain" @error="handleImageError(product.product_id)">
