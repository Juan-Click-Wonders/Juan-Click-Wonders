    ...
    <div class="bg-gray-50 min-h-screen pb-16">
        ...
        <div class="container mx-auto px-4 py-8">
            ...
            <div v-if="message" :class="[
                ...
                <svg v-if="messageType === 'error'" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <svg v-else-if="messageType === 'success'" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <!-- Empty Cart State -->
            <div v-if="!isLoading && cartItems.length === 0" class="bg-white rounded-xl shadow-md overflow-hidden p-8 text-center">
                ...
                <router-link to="/product_list" class="bg-yellow-500 hover:bg-yellow-600 text-gray-900 font-semibold px-6 py-2 rounded-lg transition-colors duration-200 inline-flex items-center">
                    ...
                    <div class="bg-white rounded-xl shadow-md overflow-hidden">
                        <div class="p-6 border-b border-gray-200">
                            <div class="flex justify-between items-center">
                                ...
                                <button @click="clearCart" class="text-gray-600 hover:text-red-600 text-sm flex items-center">
                        ...
                        <div v-for="item in cartItems" :key="item.id" class="p-6 border-b border-gray-100 hover:bg-gray-50 transition-colors">
                            <div class="flex items-center">
                                <router-link :to="`/product/${item.product.ID}`" class="w-24 h-24 rounded-lg flex items-center justify-center overflow-hidden bg-transparent border border-gray-100 hover:border-yellow-500 transition-colors">
                                ...
                                <div class="flex-grow ml-6">
                                    <router-link :to="`/product/${item.product.ID}`" class="text-lg font-semibold hover:text-yellow-600 transition-colors">{{ item.product.Name }}</router-link>
                                    <p class="text-yellow-600 font-medium">₱{{ item.product.Price.toLocaleString('en-US', {
                                        minimumFractionDigits: 2, maximumFractionDigits: 2
                                    }) }}</p>

                                    <div v-if="item.product.Inventory_Level < 5 && item.product.Inventory_Level > 0" class="text-orange-600 text-sm mt-1">
                                ...
                                <div class="flex flex-col items-end space-y-3">
                                    ...
                                    <button @click="removeFromCart(item.id)" class="text-red-600 hover:text-red-800 text-sm"
                                        ...
                                        <span v-else class="inline-flex items-center">
                                            <div class="animate-spin h-3 w-3 border-b-2 border-red-600 rounded-full mr-1"></div>
                                            Removing
                                        </span>
                                    </button>
                                </div>
                            ...
                            <div v-if="unavailableItems.includes(item.id)" class="mt-3 bg-red-50 p-3 rounded-lg text-red-600 text-sm flex items-start">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                <span>
                                    This product is no longer available. Please remove it from your cart to continue checkout.
                                </span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="p-6">
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="flex justify-between text-gray-600">
                                    ...
                                    <span class="font-medium">₱{{ shippingFee.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</span>
                                </div>
                            </div>
                            ...
                            <div class="border-t pt-4 mt-4">
                                <div class="flex justify-between font-semibold text-lg">
                                    <span>Total</span>
                                    <span class="text-xl">₱{{ total.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</span>
                                </div>
                            </div>
                            ...
                            <button @click="proceedToCheckout" 
                                ...
                                :disabled="unavailableItems.length > 0 || cartItems.length === 0 || isCheckingOut">
                            ...
                            <router-link to="/product_list" class="w-full block text-center text-gray-600 hover:text-gray-800 mt-4 text-sm">
                                Continue Shopping
                            </router-link>
                        </div>
                    </div>
...
export default {
    ...
    computed: {
        ...
        shippingFee() {
            // Free shipping for orders over 10,000 pesos
            return this.subtotal > 10000 ? 0 : 1000;
        },
        total() {
